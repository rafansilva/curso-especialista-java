...
public class CaixaEletronico {
    ...
    public static final double TARIFA_IMPRESSAO_DESMONTRATIVO = 5;

    public void transferir(Conta contaOrigem, Conta contaDestino, double valorTransferencia) {
        System.out.printf("Transferindo: R$ %.2f da conta %d/%d para conta %d/%d%n", valorTransferencia, contaOrigem.getAgencia(),
    ...
    public void imprimirDemonstrativo(Conta conta) {
        if (conta.possuiGratuidadeImpressao()) {
            System.out.println("Impressão do desmonstrativo é gratuita");
        } else {
    ...
    private static void debitarTarifaImpressaoDemonstrativo(Conta conta) {
        System.out.printf("Custo da impressão: %.2f%n", TARIFA_IMPRESSAO_DESMONTRATIVO);
        conta.sacar(TARIFA_IMPRESSAO_DESMONTRATIVO);
    }
